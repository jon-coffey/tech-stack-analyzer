...
`,
			expectedDeps: []types.Dependency{
				...
				{Type: "python", Name: "range-package", Version: ">=1.0.0,<2.0.0"},
			},
		},
	}
...
func TestPythonParser_EnhancedPyproject(t *testing.T) {
	parser := NewPythonParser()
...
dependencies = [
    "requests>=2.25.0",
    "django[bcrypt]",
    "package[extra1,extra2]>=1.0,<2.0; python_version >= '3.8'",
]`

	result := parser.ParsePyprojectTOML(input)
	expected := []types.Dependency{
		{Type: "python", Name: "requests", Version: ">=2.25.0", SourceFile: "pyproject.toml"},
		{Type: "python", Name: "django", Version: "latest", SourceFile: "pyproject.toml"},
		{Type: "python", Name: "package", Version: ">=1.0,<2.0", SourceFile: "pyproject.toml"},
	}

	require.Len(t, result, len(expected), "Should return correct number of dependencies")
